import { clearPendingVerificationCookie } from "@/src/shared/lib/auth/pending-verification-cookie";
import Link from "next/link";
import { verifyEmail } from "@/src/shared/lib/auth/verify-email";
...
export default async function EmailVerificationPage( {searchParams} : VerifyEmailPageProps ) {
      ...
      <div className="relative flex min-h-[78vh] items-center justify-center py-12">
        ...
        <section className="w-full max-w-md rounded-3xl border border-black/10 bg-white/80 p-7 shadow-xl backdrop-blur-md dark:border-white/10 dark:bg-white/5">
          <div className="mb-6">
            ...
            <h1 className="mt-4 text-3xl font-bold tracking-tight">Doğrulama Bağlantısı Geçersiz</h1>
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
              Doğrulama bağlantısı geçersiz veya süresi dolmuş olabilir. Lütfen tekrar deneyin.
            </p>
          </div>
        </section>
      </div>
