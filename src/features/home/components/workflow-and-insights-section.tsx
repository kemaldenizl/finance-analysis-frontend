      ...
      <div className="rounded-2xl border border-black/10 bg-white/75 p-6 shadow-sm dark:border-white/10 dark:bg-white/5">
        ...
        <div className="mt-5 space-y-3">
          {insightPoints.map((insight) => (
            <div
              key={insight.label}
              className="rounded-xl border border-black/15 bg-white/5 p-4 dark:border-white/10 dark:bg-white/5"
            >
              <p className="text-xs text-slate-600 dark:text-slate-300">{insight.label}</p>
              <div className="mt-1 flex items-center justify-between">
                <p className="text-lg font-semibold">{insight.value}</p>
                <span
                  className={`rounded-full px-2 py-1 text-xs ${
                    insight.trend === "up"
                      ? "bg-rose-500/20 text-rose-500 dark:text-rose-200"
                      : "bg-emerald-500/20 text-emerald-500 dark:text-emerald-200"
                  }`}
                >
